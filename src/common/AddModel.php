 ...
 * @property array $add_validate 验证器
...
trait AddModel
{
    ...
    public function add(): array
    {
        ...
        if (method_exists($this, 'addBeforeHooks') && !$this->addBeforeHooks()) {
            return $this->add_before_result;
        }

        return !Db::transaction(function () use ($model) {
            if (!method_exists($this, 'addAfterHooks')) {
                return Db::name($model)->insert($this->post);
            }

            $id = null;
            if (!empty($this->post['id'])) {
                $id = $this->post['id'];
                $result = Db::name($model)->insert($this->post);
                if (!$result) {
                    return false;
                }
            } else {
                $id = Db::name($model)->insertGetId($this->post);
            }
